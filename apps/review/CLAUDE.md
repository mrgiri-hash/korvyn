...
> ## ⚠️ SUPERSEDED 2026-08-14 — this is no longer the active front-end
>
> The main file is now **`index.html` at the repo root**, at the owner's direction. It descends from
> `apps/dashboard/`, not from this app, so the convergence plan's direction of travel is reversed —
> see [`apps/CONVERGENCE.md`](../CONVERGENCE.md) for what that costs.
>
> **Do not edit this file unless the owner names it.** Read from it freely: it is still the only home
> of the **live GL engine** (`glLedger()`, the 152-account REIT chart that foots via cash-plug and
> retained earnings, every taxonomy derived from `E_ACC`) and the deepest flux surface in the repo.
> Porting that engine into the main file is the open increment.
>
> Two things to know before lifting anything out of here: this app's type scale is **rem** and the
> main file's is **px** (convert, never paste), and this app keeps its aliases and dark ramp both on
> `:root` while the main file splits them across `html`/`body`.
>
> **Everything below describes this app as it stands and remains accurate about it** — it is the
> record of a working prototype, not a mandate for new work. Where it and the root
> [`CLAUDE.md`](../../CLAUDE.md) disagree about what the product *is*, the root wins.

The Financial Review Platform — a single self-contained HTML file (inlined wordmark + tokens, no
build step). Open it in a browser, or serve it live with the agent (below).
