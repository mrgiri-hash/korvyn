  ...
  },
  {
    ...
    run(a, env) {
      ...
      return { warnings: [], object: base(env, {
        ...
        /* §34 — the REFERENCE the source system publishes, never a fabricated deep link. This instance
           publishes none, and the trace says so rather than inventing a URL. */
        refs: { transactionId: l.key, journalId: l.journalId, account: l.account, sourceRef: `${s.erpSystem} ${s.instance} · ${s.transactionType} ${s.transactionId}` }, focus: { kind: 'transaction', id: l.key, name: `Transaction ${l.key}` }, provenance: { source: GL_SRC, snapshotId: SNAPSHOT_ID, journalLines: 1, fxRateSetId: FX_RATE_SET.id, eliminations: null, declaredInputs: [FX_RATE_SET.id, AP_EXTRACT.id] },
      }) };
    },
  },
  {
  ...
  { id: 'getFluxExplanation', domain: 'flux', permission: 'FLUX_VIEW', risk: 'READ', objectTypes: ['FLUX'], description: 'The explanation recorded on a flux line: text, author, reviewer, version, status.', params: [fluxAcct, P('period')], outputs: 'FluxExplanation; facts status, author, reviewer, text',
    run(a, env) { const p = a['period']!, i = fluxItem(env, a['account']!, p);
      ...
      return { warnings: [SEEDED], object: base(env, { type: 'FluxExplanation', title: `Explanation · ${i.name} · ${periodLabel(p)}`, periods: [p], periodLabel: periodLabel(p),
        ...
        refs: { account: i.account, explanationId: e.id, explanationVersion: String(e.version), explanationStatus: e.status, fluxItemId: i.id }, focus: { kind: 'fluxItem', id: i.id, name: i.name } }) }; } },
