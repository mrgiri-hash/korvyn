...
async function route(req: IncomingMessage, res: ServerResponse, url: string): Promise<void> {
  ...
  if (req.method === 'POST' && r === 'turn') {
    ...
    if (typeof body['sessionId'] === 'string' && !ownsSession(res, actor, body['sessionId'])) return;
    const out = await orchestrator.turn({ sessionId: body['sessionId'], request: body['request'], clarification: body['clarification'], view: body['view'], focus: body['focus'], ...('anchor' in body ? { anchor: body['anchor'] } : {}) }, actor);
  ...
  if (req.method === 'POST' && r === 'turn/stream') {
    ...
    const out = await orchestrator.turn({ sessionId: body['sessionId'], request: body['request'], clarification: body['clarification'], view: body['view'], focus: body['focus'], ...('anchor' in body ? { anchor: body['anchor'] } : {}) }, actor, { emit: (e) => write(e as unknown as Record<string, unknown>), signal: ac.signal });
